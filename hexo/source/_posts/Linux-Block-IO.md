...
LevelDB所采用的LSM（日志结构合并树）是一种较好的追加写设计方案，LSM的思想是将整个磁盘看做一个日志，在日志中存放永久性数据及其索引，每次都添加到日志的末尾。并且通过将很多小文件的存取转换为连续的大批量传输，使得对于文件系统的大多数存取都是顺序的，从而提高磁盘I/O。其具体设计可参考[LevelDB原理剖析](http://blog.dujiong.net/2017/03/27/leveldb/)
#### 文件合并和元数据优化
